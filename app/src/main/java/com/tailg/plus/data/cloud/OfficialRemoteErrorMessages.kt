...
/**
 * Port of `OfficialRemoteErrorMessages` from
 * `lib/services/official_remote_error_messages.dart` (folded into the cloud
 * package per the port plan).
 *
 * Human-facing remote-control errors (MQTT ensure/publish + HTTP cmd).
 * Token expiry / offline / broker failures must not be swallowed into opaque
 * strings that leave the user without a next step.
 */
