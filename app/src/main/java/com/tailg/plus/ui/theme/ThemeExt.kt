...
private const val THEME_TRANSITION_MS = 300

/**
 * Theme colour cross-fade.
 *
 * The whole M3 [ColorScheme] is animated role by role — 48 `animateColorAsState`
 * instances. Two properties matter for this to be affordable:
 *
 * 1. **The spec must be finite.** With the previous `spring()` (no damping
 *    ratio given) every role kept retargeting for hundreds of milliseconds.
 *    Because all 48 animations live in `TailgTheme`'s composition, every frame
 *    of that window recomposed the whole app subtree that reads
 *    `MaterialTheme.colorScheme`. A tween reaches the target colour and stops
 *    the frame clock.
 * 2. **It must honour reduce-motion.** With system animations disabled the
 *    colours snap instead of fading (duration 0).
 */
@Composable
fun ColorScheme.animateAsState(): ColorScheme {
    ...
    fun animateColor(color: Color): Color = animateColorAsState(
        targetValue = color,
        animationSpec = tween(durationMillis = if (reduceMotion) 0 else THEME_TRANSITION_MS),
